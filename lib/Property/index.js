...
class Property {
    ...
    _getPropertyType() {
        const conditions = {
            options   : this.node ? 'NODE_OPTION' : 'DEVICE_OPTION',
            telemetry : this.node ? 'NODE_TELEMETRY' : 'DEVICE_TELEMETRY'
        };

        if (this.device) return conditions[this._eventPrefix] || 'SENSOR';
    ...
    deleteRequest() {
        const payload = { type: this._getPropertyType(), deviceId: this.device.id, propertyId: this.id };

        if (this.node) payload.nodeId = this.node.id;
