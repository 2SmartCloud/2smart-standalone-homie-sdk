...
class Translator {
    ...
    parseTopic(topic, value) {
        ...
        switch (root) {
            ...
            case this.deviceSettingsTopic: {
                const typeOfSettings = map.shift();

                if (typeOfSettings === this.scenarioTopic) translated = this._translateScenario({ map, value, error });
                else translated = this._translateDeviceMap({ map, value, error });
