...
class Homie extends EventEmitter {
    constructor({ transport, debug = false, rootTopic = '', syncMaxDelay = 10000, syncResetTimeout = 1000 }) {
        ...
        this.scenarioSettingsTopic = `${DEVICE_SETTINGS_TOPIC}/${this.scenarioTopic}`;
    ...
    _updatePropertyState({ translated, property, type, key }) {
        ...
        const validBefore = property._isValid;
        ...
        if (!property._isValid) {
            if (validBefore) {
                const payload = { type: 'PROPERTY', deviceId: property.device.id, propertyId: property.id, propType: property._getPropertyType() };

                if (property.node) payload.nodeId = property.node.id;
                this.emit('events.delete.success', payload);
            }
    ...
    _updateThresholdState({ translated, scenarioId, thresholdId, key }) {
