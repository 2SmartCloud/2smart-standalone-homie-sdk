...
class Scenario {
    ...
    _updateGroupMap(data, remove = false) {
        const { type, groupId, scenarioId, propertyId } = data;
        const hash = `${type}.${scenarioId}.${propertyId}`;
    ...
    setSettingAttribute(attribute, value) {
        ...
        const translated = etl.translateObjToTopic(this._type, { [attribute]: value }, this.getTopic());
        const topic = `${this._homie.scenarioSettingsTopic}/${Object.keys(translated)[0]}`;
    ...
    publishSettingError(setting, error = { code: 'ERROR', message: 'Something went wrong' }) {
        ...
        const topic = `${this._homie.errorTopic}/${this._homie.scenarioSettingsTopic}/${Object.keys(translated)[0]}`;

        this._homie.publishToBroker(topic, JSON.stringify(error), { retain: false });
    }

    getRootSettingTopic() {
        return this._homie ? `${this._homie.scenarioSettingsTopic}/${this.getTopic()}` : this.getTopic();
    }
