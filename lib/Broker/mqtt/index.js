...
class MQTTTransport extends EventEmitter {
    ...
    publish(topic, message, options = {}, cb = () => {}) {
        ...
        this.client.publish(topic, `${message}`, { retain: this.retain, qos: options.qos || 0, ...options }, (err) => {
    ...
    subscribe(topic, cb = (this.callbacks.onSubscribe || (() => {})), options = {}) {
    ...
    handleConnect(connack) {
        this.connected = true;
        this.debug.info('MQTTTransport.handleConnect', `CONNECTED TO ${this.uri}`);
        this.emit('connect');
    }
    ...
    handleClose() {
        if (this.connected) {
            ...
            this.debug.info('MQTTTransport.handleClose', `DISCONNECTED FROM ${this.uri}`);
        }
        this.emit('close');
    }
