...
/**
 * Description for entitiesScheme structure:
 *
 * This config defines a list of 2Smart Standalone entities which are controlled by backend service.
 * Each entity stored and contolled over MQTT broker.
 *
 * rootTopic - the topic, where all entities of a single type stored at.
 * type - the entity type. Should be unique within all defined entities.
 * autoInitialize - means, that this entity will be initialized (and synced) during Homie instance initialization
 * attributes - a set of entity attributes.
 * methods - a set of custom methods that will be available for a specific entity type.
 *          Could be used to create a new methods or to override existing methods.
 *
 * Entity attribute config structure:
 *      validation - A set of LIVR rules (https://livr-spec.org/validation-rules.html) which will be used during value validation of an attribute.
 *                   No validation applied if empty.
 *      settable - Specify if attribute is settable or not. "false" by default.
 *      retained - Specify if attribute is retained in broker or not. "true" by default.
 *      description - A small description for an attribute.
 *      dataType - "string" by default but could be "json". If "json", the value will be parsed into a JSON structure.
 */
...
const entitiesScheme = [
    ...
    },
    {
        ...
        attributes     : {
            ...
            senderType : {
                validation : [ 'required', 'string', { 'one_of': [ 'device', 'heartbeat', 'backend' ] } ],
                settable   : false,
                retained   : true
            },
            senderName : {
